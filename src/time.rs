...
pub fn now(timezone: &String) -> Result<DateTime<Tz>, Error> {
...
pub fn timezone_from_str(timezone: &String) -> Result<Tz, Error> {
...
mod tests {
    ...
    fn test_timezone_from_string() {
        assert_eq!(
            timezone_from_str(&"America/Los_Angeles".to_string()),
            Ok(Tz::America__Los_Angeles),
        );

        assert_eq!(
            timezone_from_str(&"GMT -7:00".to_string()),
            Ok(Tz::Etc__GMTPlus7),
        );
    }
}
